...
自定义语义节点的价值不只是把同一张卡片换成不同颜色。设计组件时，应先根据行业任务
决定信息层级和交互，再选择视觉语言：

| 行业对象 | 应优先表达的信息 | 合适的组件结构 |
| --- | --- | --- |
| 医疗检验 | 数值、单位、参考区间、采样时间 | 检验单分栏和异常标记 |
| 农业观测 | 地块、作物、生育期、环境读数 | 田间记录和测量仪表 |
| 设备点检 | 设备编号、读数、工位、处置状态 | 工业铭牌和点检工单 |
| 安全调查 | 事件、资产、攻击阶段、证据时间 | 调查终端和证据链 |

推荐遵循以下分层：

1. Protocol 只声明业务语义、可验证属性和模型约束。
2. React/Vue 组件负责信息结构、可访问性和受控交互。
3. CSS 或业务设计系统负责排版、质感、响应式布局和状态反馈。

不要为了控制外观而给 Protocol 增加 `class`、`style`、颜色或任意布局属性。若需要为
整份文档切换视觉语言，由宿主应用根据受信任的场景或产品状态设置 `data-*` 属性，
不要使用模型输出直接控制样式。

同一个状态也不应只依靠颜色表达。例如设备停机可以同时使用 `LOCKOUT` 标签、斜纹和
结构化操作说明；检验异常可以同时展示参考区间和文字标记。这能改善可访问性，也能让
行业差异来自信息设计而不是主题色。

完整的医疗、农业、制造和网络安全示例见
[行业场景与视觉设计](./industry-scenarios.md)。
